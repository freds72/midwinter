...
function make_plyr(p,params)
	...
	-- 
	local t,bonus=params.total_t,{}
	local total_t,freeride_t=0,0
	...
	local function add_time_bonus(tb)
		t=max(t+tb*30)
		local ttl,text=8+rnd(10),"+"..tb.."s"
		add(bonus,{text=text,x=rnd(5)-10-#text/2,ttl=8+rnd(10),duration=ttl})
	end
	...
	body.update=function(self)
		...
		-- collision detection
		local hit_type,hit_actor=ground:collide(self.pos,0.2)
		if hit_type==2 then
			-- walls: insta-death
			cam:shake()
			self.dead=true
		...
		local slice,slice_extent=ground:get_track(self.pos)
		self.on_track=true
...
function menu_state()
	...
	-- 1: focus
	-- 2: selected
	function draw_box(s,x,y,c,mode,freeride)
		...
		if freeride then
			palt(0,false)
			palt(14,true)
			spr(102,x-8,y+h-34,2,2)
			rectfill(x-18,y+h-17,x+24,y+h-12,10)
			print("FREERIDING",x-16,y+h-17,0)
		end
		pal()
	end
   
	local records={30*30,20*30,15*30}
	for i=0,2 do
		local t=dget(i)
		-- avoid bogus data
	...
	local tree_prop,bush_prop,cow_prop={sx=112,sy=16,sfx={9,10}},{sx=96,sy=32,sfx={9,10}},{sx=80,sy=32,sfx={4}}
	local panels={
		{text="marmottes",c=1,params={dslot=0,slope=1.5,tracks=1,bonus_t=2,total_t=30*30,record_t=records[1],props={tree_prop},props_rate=0.85,show_help=true}},
		{text="biquettes",c=8,freeride=true,params={dslot=1,slope=2,tracks=2,bonus_t=1.5,total_t=20*30,record_t=records[2],props={tree_prop,bush_prop},props_rate=0.87,}},
		{text="chamois",c=0,freeride=true,params={dslot=2,slope=3,bonus_t=1.5,total_t=15*30,record_t=records[3],props={tree_prop,tree_prop,tree_prop,cow_prop},props_rate=0.92,}}
	}
	local sel,sel_tgt,blink=0,0,false
	local pulse_colors={[0]=0,1,13,12}

	ground=make_ground({slope=0,tracks=1,props_rate=2})
		...
		draw=function()
			...
			for i=1,#panels do
				...
				if v[3]>0 then
					...
					local p=panels[i]
					draw_box(p.text,x0+32,y0,p.c,mode,p.freeride)
				end
				a+=da
			end
			...
			if sel==sel_tgt then
				...
				print(s,64-2*#s,96,pulse_colors[flr(3*abs(cos(time())))])
			end
	...
	return {
		...
		draw=function()
			...
			if plyr then
				...
				-- 
				local t,bonus=plyr:score()
				printb(time_tostr(t),nil,4,10,9,1)

				for i=1,#bonus do
					local b=bonus[i]					
					if(b.ttl/b.duration>0.5 or t%2==0) printb(b.text,64+b.x,40+b.ttl,10,9,1)
				end
				...
				if plyr.on_track and (32*time())%8<4 then
					...
					if dx<-0.1 then
						sspr(32,48,16,16,2,32,32,32)
					elseif dx>0.1 then
						sspr(32,48,16,16,96,32,32,32,true)
					end
				end
				spr(72,56,12,2,2)
...
function make_ground(params)
	...
	local left_pole,right_pole={sx=48,sy=0},{sx=48,sy=16}
	...
	local function make_slice(y)
		...
		for i=0,nx-1 do
			...
			if rnd()>params.props_rate then
				local prop=pick(params.props)
				actors[i]={sx=prop.sx,sy=prop.sy,sfx=prop.sfx,r=1.2}
			end
		end
		...
		for _,t in pairs(tracks) do
			...
			if t.main then
				...
				if slice_id%8==0 then
					is_checkpoint={}
					actors[i0]={sx=left_pole.sx,sy=left_pole.sy}
					actors[i1]={sx=right_pole.sx,sy=right_pole.sy}					
				end
			else
				-- side tracks
				for i=i0,i1 do				
				...
				-- coins
				actors[ii]={strip=coins_strip,speed=3,r=1,score=1}
			end
		end
	...
	return {
		plyr_pos={slices[plyr_z_index].x,2,plyr_z_index*dz},
...
__gfx__

...

...

